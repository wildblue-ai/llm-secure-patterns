...
```
Secure External Ingestion — OWASP LLM01/LLM10. Pick a level (each builds on the last):

A) Low — no added cost. Strip HTML tags/comments + remove zero-width chars + truncate to a token budget.
   Misses any encoded payload (Base64, ROT13, homoglyphs).
B) Moderate (recommended) — no added cost. + normalize/decode encodings before filtering + wrap content in UNTRUSTED delimiters + strip metadata.
   Naive and encoded injection mitigated; semantic injection remains.
C) High — +1 classifier call/req (estimated). + secondary injection classifier + MIME-type validation + per-trust-tier credentials.
